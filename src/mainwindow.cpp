...
void MainWindow::fillSetNames(const QHash<QString, QString> &setNames) {
  for (int i = 0, iEnd = m_setsModel->rowCount(); i < iEnd; ++i) {
    const QString setToFind =
        m_setsModel->index(i, 0).data(Qt::UserRole).toString();
...
void MainWindow::onDownloaded17LRatings(const QString &set,
                                        const QSet<SeventeenCard> &ratings) {
  Q_ASSERT(!ratings.isEmpty());
  const auto ratingComparison = [this](const SeventeenCard &a,
                                       const SeventeenCard &b) -> bool {
    return ratingValue(a) < ratingValue(b);
  };
  const auto minMaxRtg =
      std::minmax_element(ratings.cbegin(), ratings.cend(), ratingComparison);
  ...
  for (int i = 0, iEnd = m_ratingsModel->rowCount(); i < iEnd; ++i) {
    ...
    if (m_ratingsModel->index(i, RatingsModel::rmcSet).data().toString() != set)
      continue;
    const auto rtgIter = ratings.constFind(SeventeenCard(
        m_ratingsModel->index(i, RatingsModel::rmcName).data().toString()));
    if (rtgIter == ratings.constEnd())
      continue;
    m_ratingsModel->setData(
        m_ratingsModel->index(i, RatingsModel::rmcRating),
        qRound(10.0 * (ratingValue(*rtgIter) - minRtgValue) /
               ratingDenominator));
    m_ratingsModel->setData(m_ratingsModel->index(i, RatingsModel::rmcNote),
                            commentString(*rtgIter));
  }
}
...
MainWindow::MainWindow(QWidget *parent)
    : QWidget(parent), m_error(NoError), ui(new Ui::MainWindow) {
  ui->setupUi(this);
  ...
  ui->ratingsView->horizontalHeader()->setSectionResizeMode(
      QHeaderView::ResizeToContents);
  ui->ratingsView->sortByColumn(RatingsModel::rmcName, Qt::AscendingOrder);
  ui->ratingsView->setItemDelegateForColumn(RatingsModel::rmcRating,
                                            new RatingsDelegate(this));
  ...
  connect(ui->ratingBasedButton, &QPushButton::clicked, this, []() {
    QDesktopServices::openUrl(QUrl::fromUserInput(
        QStringLiteral("https://www.17lands.com/metrics_definitions")));
  });
  ...
  connect(ui->retryBasicDownloadButton, &QPushButton::clicked, this,
          &MainWindow::retrySetsDownload);
  connect(ui->retryTemplateButton, &QPushButton::clicked, this,
          &MainWindow::retryTemplateDownload);
  connect(ui->downloadButton, &QPushButton::clicked, this,
          &MainWindow::do17Ldownload);
  connect(ui->uploadButton, &QPushButton::clicked, this,
          &MainWindow::doMtgahUpload);
  connect(ui->allSetsButton, &QPushButton::clicked, this,
          &MainWindow::selectAllSets);
  connect(ui->noSetButton, &QPushButton::clicked, this,
          &MainWindow::selectNoSets);
  connect(m_worker, &Worker::setsMTGAH, this, &MainWindow::fillSets);
  connect(m_worker, &Worker::downloadSetsMTGAHFailed, this,
          &MainWindow::onMTGAHSetsError);
  connect(m_worker, &Worker::setsScryfall, this, &MainWindow::fillSetNames);
  connect(m_worker, &Worker::downloadSetsScryfallFailed, this,
          &MainWindow::onScryfallSetsError);
  ...
  connect(m_worker, &Worker::customRatingTemplate, this,
          &MainWindow::onCustomRatingsTemplateDownloaded);
  connect(m_worker, &Worker::customRatingTemplateFailed, this,
          &MainWindow::onTemplateDownloadFailed);
  connect(m_worker, &Worker::downloaded17LRatings, this,
          &MainWindow::onDownloaded17LRatings);
  connect(m_worker, &Worker::downloadedAll17LRatings, this,
          &MainWindow::onDownloadedAll17LRatings);
  connect(m_setsModel, &QAbstractItemModel::dataChanged, this,
          [this](const QModelIndex &, const QModelIndex &,
                 const QVector<int> &roles) {
            if (roles.isEmpty() || roles.contains(Qt::CheckStateRole))
              updateRatingsFiler();
          });
  connect(m_ratingsModel, &QAbstractItemModel::modelReset, this,
          &MainWindow::updateRatingsFiler);
  m_worker->downloadSetsMTGAH();
}

MainWindow::~MainWindow() { delete ui; }

MainWindow::CurrentErrors MainWindow::errors() const { return m_error; }
...
void MainWindow::retranslateUi() {
  ...
  if (m_error & LoginError)
    errorStrings.append(tr(
        "Login Failed! Check your username, password or internet connection"));
  if (m_error & LogoutError)
    errorStrings.append(tr("Logout Failed! Check your internet connection"));
  if (m_error & MTGAHSetsError)
    errorStrings.append(
        tr("Error downloading sets info! Check your internet connection"));
  if (m_error & RatingTemplateFailed)
    errorStrings.append(tr(
        "Error downloading ratings template! Check your internet connection"));
  ...
  m_SLMetricsModel->item(SLseen_count)
      ->setData(tr("Number Seen (%1)").arg(SLcodes.at(SLseen_count)),
                Qt::DisplayRole);
  m_SLMetricsModel->item(SLavg_seen)
      ->setData(tr("Average Last Seen At (%1)").arg(SLcodes.at(SLavg_seen)),
                Qt::DisplayRole);
  m_SLMetricsModel->item(SLpick_count)
      ->setData(tr("Number Picked (%1)").arg(SLcodes.at(SLpick_count)),
                Qt::DisplayRole);
  m_SLMetricsModel->item(SLavg_pick)
      ->setData(tr("Average Taken At (%1)").arg(SLcodes.at(SLavg_pick)),
                Qt::DisplayRole);
  m_SLMetricsModel->item(SLgame_count)
      ->setData(tr("Number of Games Played (%1)").arg(SLcodes.at(SLgame_count)),
                Qt::DisplayRole);
  m_SLMetricsModel->item(SLwin_rate)
      ->setData(tr("Games Played Win Rate (%1)").arg(SLcodes.at(SLwin_rate)),
                Qt::DisplayRole);
  m_SLMetricsModel->item(SLopening_hand_game_count)
      ->setData(tr("Number of Games in Opening Hand (%1)")
                    .arg(SLcodes.at(SLopening_hand_game_count)),
                Qt::DisplayRole);
  m_SLMetricsModel->item(SLopening_hand_win_rate)
      ->setData(tr("Opening Hand Win Rate (%1)")
                    .arg(SLcodes.at(SLopening_hand_win_rate)),
                Qt::DisplayRole);
  m_SLMetricsModel->item(SLdrawn_game_count)
      ->setData(
          tr("Number of Games Drawn (%1)").arg(SLcodes.at(SLdrawn_game_count)),
          Qt::DisplayRole);
  m_SLMetricsModel->item(SLdrawn_win_rate)
      ->setData(
          tr("Games Drawn Win Rate (%1)").arg(SLcodes.at(SLdrawn_win_rate)),
          Qt::DisplayRole);
  m_SLMetricsModel->item(SLever_drawn_game_count)
      ->setData(tr("Number of Games In Hand (%1)")
                    .arg(SLcodes.at(SLever_drawn_game_count)),
                Qt::DisplayRole);
  m_SLMetricsModel->item(SLever_drawn_win_rate)
      ->setData(tr("Games in Hand Win Rate (%1)")
                    .arg(SLcodes.at(SLever_drawn_win_rate)),
                Qt::DisplayRole);
  m_SLMetricsModel->item(SLnever_drawn_game_count)
      ->setData(tr("Number of Games Not Drawn (%1)")
                    .arg(SLcodes.at(SLnever_drawn_game_count)),
                Qt::DisplayRole);
  m_SLMetricsModel->item(SLnever_drawn_win_rate)
      ->setData(tr("Games Not Drawn Win Rate (%1)")
                    .arg(SLcodes.at(SLnever_drawn_win_rate)),
                Qt::DisplayRole);
  m_SLMetricsModel->item(SLdrawn_improvement_win_rate)
      ->setData(tr("Improvement When Drawn (%1)")
                    .arg(SLcodes.at(SLdrawn_improvement_win_rate)),
                Qt::DisplayRole);
}
...
QString MainWindow::commentString(const SeventeenCard &card) const {
  QStringList result;
  if (m_SLMetricsModel->index(SLseen_count, 0)
          .data(Qt::CheckStateRole)
          .toInt() == Qt::Checked)
    result.append(SLcodes.at(SLseen_count) + QLatin1Char(':') +
                  locale().toString(card.seen_count));
  if (m_SLMetricsModel->index(SLavg_seen, 0).data(Qt::CheckStateRole).toInt() ==
      Qt::Checked)
    result.append(SLcodes.at(SLavg_seen) + QLatin1Char(':') +
                  locale().toString(card.avg_seen, 'f', 2));
  if (m_SLMetricsModel->index(SLpick_count, 0)
          .data(Qt::CheckStateRole)
          .toInt() == Qt::Checked)
    result.append(SLcodes.at(SLpick_count) + QLatin1Char(':') +
                  locale().toString(card.pick_count));
  if (m_SLMetricsModel->index(SLavg_pick, 0).data(Qt::CheckStateRole).toInt() ==
      Qt::Checked)
    result.append(SLcodes.at(SLavg_pick) + QLatin1Char(':') +
                  locale().toString(card.avg_pick, 'f', 2));
  if (m_SLMetricsModel->index(SLgame_count, 0)
          .data(Qt::CheckStateRole)
          .toInt() == Qt::Checked)
    result.append(SLcodes.at(SLgame_count) + QLatin1Char(':') +
                  locale().toString(card.game_count));
  if (m_SLMetricsModel->index(SLwin_rate, 0).data(Qt::CheckStateRole).toInt() ==
      Qt::Checked)
    result.append(SLcodes.at(SLwin_rate) + QLatin1Char(':') +
                  locale().toString(card.win_rate * 100.0, 'f', 2) +
                  locale().percent());
  if (m_SLMetricsModel->index(SLopening_hand_game_count, 0)
          .data(Qt::CheckStateRole)
          .toInt() == Qt::Checked)
    result.append(SLcodes.at(SLopening_hand_game_count) + QLatin1Char(':') +
                  locale().toString(card.opening_hand_game_count));
  if (m_SLMetricsModel->index(SLopening_hand_win_rate, 0)
          .data(Qt::CheckStateRole)
          .toInt() == Qt::Checked)
    result.append(
        SLcodes.at(SLopening_hand_win_rate) + QLatin1Char(':') +
        locale().toString(card.opening_hand_win_rate * 100.0, 'f', 2) +
        locale().percent());
  if (m_SLMetricsModel->index(SLdrawn_game_count, 0)
          .data(Qt::CheckStateRole)
          .toInt() == Qt::Checked)
    result.append(SLcodes.at(SLdrawn_game_count) + QLatin1Char(':') +
                  locale().toString(card.drawn_game_count));
  if (m_SLMetricsModel->index(SLdrawn_win_rate, 0)
          .data(Qt::CheckStateRole)
          .toInt() == Qt::Checked)
    result.append(SLcodes.at(SLdrawn_win_rate) + QLatin1Char(':') +
                  locale().toString(card.drawn_win_rate * 100.0, 'f', 2) +
                  locale().percent());
  if (m_SLMetricsModel->index(SLever_drawn_game_count, 0)
          .data(Qt::CheckStateRole)
          .toInt() == Qt::Checked)
    result.append(SLcodes.at(SLever_drawn_game_count) + QLatin1Char(':') +
                  locale().toString(card.ever_drawn_game_count));
  if (m_SLMetricsModel->index(SLever_drawn_win_rate, 0)
          .data(Qt::CheckStateRole)
          .toInt() == Qt::Checked)
    result.append(SLcodes.at(SLever_drawn_win_rate) + QLatin1Char(':') +
                  locale().toString(card.ever_drawn_win_rate * 100.0, 'f', 2) +
                  locale().percent());
  if (m_SLMetricsModel->index(SLnever_drawn_game_count, 0)
          .data(Qt::CheckStateRole)
          .toInt() == Qt::Checked)
    result.append(SLcodes.at(SLnever_drawn_game_count) + QLatin1Char(':') +
                  locale().toString(card.never_drawn_game_count));
  if (m_SLMetricsModel->index(SLnever_drawn_win_rate, 0)
          .data(Qt::CheckStateRole)
          .toInt() == Qt::Checked)
    result.append(SLcodes.at(SLnever_drawn_win_rate) + QLatin1Char(':') +
                  locale().toString(card.never_drawn_win_rate * 100.0, 'f', 2) +
                  locale().percent());
  if (m_SLMetricsModel->index(SLdrawn_improvement_win_rate, 0)
          .data(Qt::CheckStateRole)
          .toInt() == Qt::Checked)
    result.append(
        SLcodes.at(SLdrawn_improvement_win_rate) + QLatin1Char(':') +
        locale().toString(card.drawn_improvement_win_rate * 100.0, 'f', 2) +
        locale().percent());
