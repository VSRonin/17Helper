...
void MainWindow::changeLanguage(const QLocale &loc)
{
    ...
    for (const QString &translationUnit : {QStringLiteral("qt"), QStringLiteral("17Helper"), QStringLiteral("17HelperUI")}) {
        ...
        for (const QString &translationPrefix: {QStringLiteral(":/i18n"), QStringLiteral("translations")})
        if (translator->load(loc, translationUnit, QLatin1String("_"), translationPrefix)) {
