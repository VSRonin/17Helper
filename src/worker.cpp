...
Worker::Worker(QObject *parent)
    : QObject(parent), m_nam(new QNetworkAccessManager(this)),
      m_SLrequestOutstanding(0), m_MTGAHrequestOutstanding(0) {
  ...
  connect(requestTimer, &QTimer::timeout, this,
          &Worker::processMTGAHrequestQueue);
  requestTimer->start();
}
...
void Worker::tryLogin(const QString &userName, const QString &password) {
  ...
  const QUrl loginUrl = QUrl::fromUserInput(
      QStringLiteral("https://mtgahelper.com/api/Account/Signin?email=") +
      userName + QStringLiteral("&password=") + password);
...
void Worker::logOut() {
  const QUrl setsUrl = QUrl::fromUserInput(
      QStringLiteral("https://mtgahelper.com/api/Account/Signout"));
...
void Worker::downloadSetsMTGAH() {
  const QUrl setsUrl = QUrl::fromUserInput(
      QStringLiteral("https://mtgahelper.com/api/Misc/Sets"));
  QNetworkReply *reply = m_nam->get(QNetworkRequest(setsUrl));
  connect(reply, &QNetworkReply::errorOccurred, this,
          &Worker::downloadSetsMTGAHFailed);
  connect(reply, &QNetworkReply::finished, reply, &QNetworkReply::deleteLater);
  connect(reply, &QNetworkReply::finished, this, [reply, this]() -> void {
    if (reply->error() != QNetworkReply::NoError)
      return;
    ...
    QJsonParseError parseErr;
    QJsonDocument setsDocument =
        QJsonDocument::fromJson(reply->readAll(), &parseErr);
    if (parseErr.error != QJsonParseError::NoError ||
        !setsDocument.isObject()) {
      emit downloadSetsMTGAHFailed();
      return;
    }
    ...
    for (auto i = setsArray.cbegin(), iEnd = setsArray.cend(); i != iEnd; ++i) {
      const QString setStr =
          i->toObject()[QLatin1String("name")].toString().toUpper();
      if (!setStr.isEmpty())
...
void Worker::downloadSetsScryfall() {
  const QUrl setsUrl =
      QUrl::fromUserInput(QStringLiteral("https://api.scryfall.com/sets"));
  QNetworkReply *reply = m_nam->get(QNetworkRequest(setsUrl));
  connect(reply, &QNetworkReply::errorOccurred, this,
          &Worker::downloadSetsScryfallFailed);
  connect(reply, &QNetworkReply::finished, reply, &QNetworkReply::deleteLater);
  connect(reply, &QNetworkReply::finished, this, [reply, this]() -> void {
    if (reply->error() != QNetworkReply::NoError)
      return;
    ...
    QJsonParseError parseErr;
    const QJsonDocument setsDocument =
        QJsonDocument::fromJson(reply->readAll(), &parseErr);
    if (parseErr.error != QJsonParseError::NoError ||
        !setsDocument.isObject()) {
      emit downloadSetsScryfallFailed();
    ...
    for (auto i = setsArray.cbegin(), iEnd = setsArray.cend(); i != iEnd; ++i) {
      ...
      const QString setStr =
          setObj[QLatin1String("code")].toString().trimmed().toUpper();
      if (setStr.isEmpty())
        continue;
      if (!setNames.contains(setStr))
        setNames.insert(setStr,
                        setObj[QLatin1String("name")].toString().trimmed());
    }
    if (setNames.isEmpty()) {
      emit downloadSetsScryfallFailed();
      return;
...
void Worker::getCustomRatingTemplate() {
  const QUrl setsUrl = QUrl::fromUserInput(QStringLiteral(
      "https://mtgahelper.com/api/User/customDraftRatingsForDisplay"));
  QNetworkReply *reply = m_nam->get(QNetworkRequest(setsUrl));
  connect(reply, &QNetworkReply::errorOccurred, this,
          &Worker::customRatingTemplateFailed);
  connect(reply, &QNetworkReply::finished, reply, &QNetworkReply::deleteLater);
  connect(reply, &QNetworkReply::finished, this, [reply, this]() -> void {
    if (reply->error() != QNetworkReply::NoError)
      return;
    ...
    const QJsonDocument ratingsDocument =
        QJsonDocument::fromJson(reply->readAll(), &parseErr);
    if (parseErr.error != QJsonParseError::NoError ||
        !ratingsDocument.isArray()) {
      emit customRatingTemplateFailed();
      return;
    }
    decltype(m_ratingsTemplate) rtgsTemplate;
    const QJsonArray ratingsArray = ratingsDocument.array();
    for (auto i = ratingsArray.cbegin(), iEnd = ratingsArray.cend(); i != iEnd;
         ++i) {
      QCoreApplication::processEvents();
      if (!i->isObject())
        continue;
      const QJsonObject ratingObject = i->toObject();
      const QJsonObject cardObject =
          ratingObject[QLatin1String("card")].toObject();
      ...
      if (std::find_if(rtgsTemplate.cbegin(), rtgsTemplate.cend(),
                       [idArenaVal](const MtgahCard &card) -> bool {
                         return idArenaVal == card.id_arena;
                       }) != rtgsTemplate.cend())
        continue;
      const QString setStr =
          cardObject[QLatin1String("set")].toString().trimmed().toUpper();
      if (setStr.isEmpty())
        continue;
...
void Worker::get17LRatings(const QStringList &sets, const QString &format) {
  ...
  for (const QString &set : sets) {
    const QUrl ratingsUrl = QUrl::fromUserInput(
        QStringLiteral("https://www.17lands.com/card_ratings/data?expansion=") +
        set + QLatin1String("&format=") + format);
...
void Worker::processSLrequestQueue() {
  if (m_SLrequestQueue.isEmpty())
    return;
  const std::pair<QString, QNetworkRequest> currReq =
      m_SLrequestQueue.takeFirst();
  const QString currSet = currReq.first;
  ++m_SLrequestOutstanding;
  QNetworkReply *reply = m_nam->get(currReq.second);
  connect(reply, &QNetworkReply::errorOccurred, this,
          &Worker::failed17LRatings);
  connect(reply, &QNetworkReply::finished, reply, &QNetworkReply::deleteLater);
  connect(
      reply, &QNetworkReply::finished, this, [reply, this, currSet]() -> void {
        if (reply->error() != QNetworkReply::NoError)
          return;
        ...
        const QJsonArray ratingsArray = ratingsDocument.array();
        for (auto i = ratingsArray.cbegin(), iEnd = ratingsArray.cend();
             i != iEnd; ++i) {
          QCoreApplication::processEvents();
          if (!i->isObject())
            continue;
          ...
          const QString nameStr =
              ratingObject[QLatin1String("name")].toString();
          if (nameStr.isEmpty())
            continue;
          SeventeenCard card;
          card.name = nameStr;
          ...
          card.opening_hand_game_count =
              ratingObject[QLatin1String("opening_hand_game_count")].toInt();
          card.opening_hand_win_rate =
              ratingObject[QLatin1String("opening_hand_win_rate")].toDouble();
          card.drawn_game_count =
              ratingObject[QLatin1String("drawn_game_count")].toInt();
          card.drawn_win_rate =
              ratingObject[QLatin1String("drawn_win_rate")].toDouble();
          card.ever_drawn_game_count =
              ratingObject[QLatin1String("ever_drawn_game_count")].toInt();
          card.ever_drawn_win_rate =
              ratingObject[QLatin1String("ever_drawn_win_rate")].toDouble();
          card.never_drawn_game_count =
              ratingObject[QLatin1String("never_drawn_game_count")].toInt();
          card.never_drawn_win_rate =
              ratingObject[QLatin1String("never_drawn_win_rate")].toDouble();
          card.drawn_improvement_win_rate =
              ratingObject[QLatin1String("drawn_improvement_win_rate")]
                  .toDouble();
        ...
        emit download17LRatingsProgress(m_SLrequestQueue.size() +
                                        m_SLrequestOutstanding);
      });
}
...
void Worker::uploadRatings(const QStringList &sets) {
  for (const QString &set : sets) {
    ...
    for (auto i = cardsRange.first; i != cardsRange.second; ++i) {
      ...
      const QUrl ratingUrl = QUrl::fromUserInput(
          QStringLiteral("https://mtgahelper.com/api/User/CustomDraftRating"));
      QNetworkRequest ratingReq(ratingUrl);
      ratingReq.setHeader(QNetworkRequest::ContentTypeHeader,
...
void Worker::processMTGAHrequestQueue() {
  if (m_MTGAHrequestQueue.isEmpty())
    return;
  const std::pair<MtgahCard, QNetworkRequest> currReq =
      m_MTGAHrequestQueue.takeFirst();
  ...
  QNetworkReply *reply = m_nam->put(
      currReq.second, QJsonDocument(cardData).toJson(QJsonDocument::Compact));
  connect(reply, &QNetworkReply::errorOccurred, this,
          std::bind(&Worker::failedUploadRating, this, currCard));
  connect(reply, &QNetworkReply::finished, reply, &QNetworkReply::deleteLater);
  connect(reply, &QNetworkReply::finished, this,
          [reply, this, currCard]() -> void {
            if (reply->error() != QNetworkReply::NoError)
              return;
            if (reply->attribute(QNetworkRequest::HttpStatusCodeAttribute)
                    .toInt() != 200) {
            ...
            emit ratingsUploadProgress(m_SLrequestQueue.size() +
                                       m_MTGAHrequestOutstanding);
          });
}
