...
void CustomRatingsDelegate::destroyEditor(QWidget *editor, const QModelIndex &index) const
{
    currentEditor = nullptr;
...
void CustomRatingsDelegate::initStyleOption(QStyleOptionViewItem *option, const QModelIndex &index) const
{
    ...
    if (!value.isValid()) {
        ...
        option->text = displayText(-1, option->locale);
    }
}
...
QWidget *CustomRatingsDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    myButton->setIcon(QIcon(QStringLiteral(":icons/trash.svg")));
...
QStyleOptionButton CustomRatingsDelegate::buttonOptions(const QStyleOptionViewItem &option, bool skipRct) const
{
    ...
    buttonOption.icon = QIcon(QStringLiteral(":icons/trash.svg"));
...
QRect CustomRatingsDelegate::buttonRect(const QStyleOptionViewItem &option) const
{
    ...
    return QRect(option.rect.left() + option.rect.width() - buttonSize.width(), option.rect.top(), buttonSize.width(),
                 qMax(buttonSize.height(), option.rect.height()));
}
